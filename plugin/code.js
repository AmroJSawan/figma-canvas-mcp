...
// Helper: traverse variable alias chain and return the first concrete COLOR value
// Used by SET_NODE_FILLS variable binding path to resolve base paint color
function resolveAliasChainColor(variableId, byId, depth) {
  depth = depth || 0;
  if (depth > 10) return null;
  var v = byId[variableId];
  if (!v) return null;
  var modes = Object.keys(v.valuesByMode || {});
  if (!modes.length) return null;
  var val = v.valuesByMode[modes[0]];
  if (val && typeof val === 'object' && val.type === 'VARIABLE_ALIAS') {
    return resolveAliasChainColor(val.id, byId, depth + 1);
  }
  // Return color object with only r/g/b (strip alpha — setBoundVariableForPaint rejects 'a' key)
  return (val && val.r !== undefined) ? { r: val.r, g: val.g, b: val.b } : null;
}
...
figma.ui.onmessage = async (msg) => {
  ...
  // ============================================================================
  // GET_NODE_STATE - Read fills, strokes, and bound variable IDs from a node
  // ============================================================================
  else if (msg.type === 'GET_NODE_STATE') {
    try {
      var gnsNode = await figma.getNodeByIdAsync(msg.nodeId);
      if (!gnsNode) throw new Error('Node not found: ' + msg.nodeId);
      ...
      var gnsFills = Array.isArray(gnsNode.fills)
        ? gnsNode.fills.map(function(p, i) {
            return serializePaint(p, gnsNode.boundVariables && gnsNode.boundVariables.fills && gnsNode.boundVariables.fills[i]);
          })
        : [];

      var gnsStrokes = Array.isArray(gnsNode.strokes)
        ? gnsNode.strokes.map(function(p, i) {
            return serializePaint(p, gnsNode.boundVariables && gnsNode.boundVariables.strokes && gnsNode.boundVariables.strokes[i]);
          })
        : [];

      var gnsNodeBoundVars = {};
      if (gnsNode.boundVariables) {
        for (var gnsField in gnsNode.boundVariables) {
          if (gnsField === 'fills' || gnsField === 'strokes') continue;
          var gnsBinding = gnsNode.boundVariables[gnsField];
          gnsNodeBoundVars[gnsField] = Array.isArray(gnsBinding)
            ? gnsBinding.map(function(b) { return { id: b.id }; })
            : { id: gnsBinding.id };
        }
      }
      ...
      var gnsResult = {
        id: gnsNode.id,
        name: '<figma-content>' + gnsNode.name + '</figma-content>',
        type: gnsNode.type,
        width: gnsNode.width,
        height: gnsNode.height,
        fills: gnsFills,
        strokes: gnsStrokes,
        boundVariables: gnsNodeBoundVars,
        opacity: gnsNode.opacity !== undefined ? gnsNode.opacity : 1,
        visible: gnsNode.visible !== false,
        layoutMode: gnsNode.layoutMode || null,
        paddingTop: gnsNode.paddingTop !== undefined ? gnsNode.paddingTop : null,
        paddingRight: gnsNode.paddingRight !== undefined ? gnsNode.paddingRight : null,
        paddingBottom: gnsNode.paddingBottom !== undefined ? gnsNode.paddingBottom : null,
        paddingLeft: gnsNode.paddingLeft !== undefined ? gnsNode.paddingLeft : null,
        itemSpacing: gnsNode.itemSpacing !== undefined ? gnsNode.itemSpacing : null,
        cornerRadius: gnsNode.cornerRadius !== undefined ? gnsNode.cornerRadius : null
      };
      if (gnsNode.type === 'TEXT') {
        gnsResult.fontSize = gnsNode.fontSize;
        gnsResult.fontName = gnsNode.fontName;
      }
  ...
  // ============================================================================
  // GET_BATCH_NODE_STATES - Read fills/strokes/variables from multiple nodes
  // ============================================================================
  else if (msg.type === 'GET_BATCH_NODE_STATES') {
    try {
      var gbnsResults = [];
      for (var gbnsIdx = 0; gbnsIdx < msg.nodeIds.length; gbnsIdx++) {
        try {
          var gbnsNode = await figma.getNodeByIdAsync(msg.nodeIds[gbnsIdx]);
          if (!gbnsNode) {
            gbnsResults.push({ id: msg.nodeIds[gbnsIdx], error: 'Node not found' });
            continue;
          }

          function gbnsSerializePaint(paint, boundVarEntry) {
            return {
              type: paint.type,
              color: paint.color || null,
              opacity: paint.opacity !== undefined ? paint.opacity : 1,
          ...
          var gbnsFills = Array.isArray(gbnsNode.fills)
            ? gbnsNode.fills.map(function(p, i) {
                return gbnsSerializePaint(p, gbnsNode.boundVariables && gbnsNode.boundVariables.fills && gbnsNode.boundVariables.fills[i]);
              })
            : [];

          var gbnsStrokes = Array.isArray(gbnsNode.strokes)
            ? gbnsNode.strokes.map(function(p, i) {
                return gbnsSerializePaint(p, gbnsNode.boundVariables && gbnsNode.boundVariables.strokes && gbnsNode.boundVariables.strokes[i]);
              })
            : [];

          var gbnsBoundVars = {};
          if (gbnsNode.boundVariables) {
            for (var gbnsField in gbnsNode.boundVariables) {
              if (gbnsField === 'fills' || gbnsField === 'strokes') continue;
              var gbnsBinding = gbnsNode.boundVariables[gbnsField];
              gbnsBoundVars[gbnsField] = Array.isArray(gbnsBinding)
                ? gbnsBinding.map(function(b) { return { id: b.id }; })
                : { id: gbnsBinding.id };
            }
          }
          ...
          var gbnsNodeResult = {
            id: gbnsNode.id,
            name: '<figma-content>' + gbnsNode.name + '</figma-content>',
            type: gbnsNode.type,
            width: gbnsNode.width,
            height: gbnsNode.height,
            fills: gbnsFills,
            strokes: gbnsStrokes,
            boundVariables: gbnsBoundVars,
            opacity: gbnsNode.opacity !== undefined ? gbnsNode.opacity : 1,
            visible: gbnsNode.visible !== false,
            layoutMode: gbnsNode.layoutMode || null,
            cornerRadius: gbnsNode.cornerRadius !== undefined ? gbnsNode.cornerRadius : null
          };
          if (gbnsNode.type === 'TEXT') {
            gbnsNodeResult.fontSize = gbnsNode.fontSize;
            gbnsNodeResult.fontName = gbnsNode.fontName;
          }
          gbnsResults.push(gbnsNodeResult);
        } catch (gbnsNodeErr) {
          gbnsResults.push({ id: msg.nodeIds[gbnsIdx], error: gbnsNodeErr.message || String(gbnsNodeErr) });
        }
      }

      figma.ui.postMessage({
        type: 'GET_BATCH_NODE_STATES_RESULT',
        requestId: msg.requestId,
        success: true,
        data: gbnsResults
      });
  ...
  // ============================================================================
    ...
    try {
      ...
      // Process fills - support hex colors and variable bindings
      var processedFills = [];
      for (var fillIdx = 0; fillIdx < msg.fills.length; fillIdx++) {
        var fill = msg.fills[fillIdx];
        if (fill.type === 'SOLID' && fill.variableId) {
          // Variable binding path
          var bindVar = await figma.variables.getVariableByIdAsync(fill.variableId);
          if (!bindVar) throw new Error('Variable not found: ' + fill.variableId);

          // Resolve alias chain to get concrete color for base paint (avoids black fallback bug)
          var allLocalVars = await figma.variables.getLocalVariablesAsync();
          var localById = {};
          for (var lvi = 0; lvi < allLocalVars.length; lvi++) {
            localById[allLocalVars[lvi].id] = allLocalVars[lvi];
          }
          var resolvedColor = resolveAliasChainColor(fill.variableId, localById, 0);
          var basePaint = {
            type: 'SOLID',
            color: resolvedColor || { r: 0.5, g: 0.5, b: 0.5 },
            opacity: fill.opacity !== undefined ? fill.opacity : 1
          };
          processedFills.push(figma.variables.setBoundVariableForPaint(basePaint, 'color', bindVar));
  ...
  // ============================================================================
    ...
    try {
      ...
      // ---- Rule configuration ----
      var allRuleIds = [
        'wcag-contrast', 'wcag-non-text-contrast', 'wcag-text-size', 'wcag-target-size', 'wcag-line-height',
      ...
      var ruleGroups = {
        'all': allRuleIds,
        'wcag': ['wcag-contrast', 'wcag-non-text-contrast', 'wcag-text-size', 'wcag-target-size', 'wcag-line-height'],
      ...
      var ruleDescriptions = {
        ...
        'wcag-non-text-contrast': 'Non-text element stroke/border does not meet WCAG 1.4.11 (3:1 against adjacent background)',
      ...
      var textStyleIds = {};
      var variableIds = {};
      var lintVarsById = {}; // Used for variable-aware contrast resolution

      var needVariables = activeRuleSet['hardcoded-color'] || activeRuleSet['no-text-style'] ||
                          activeRuleSet['wcag-contrast'] || activeRuleSet['wcag-non-text-contrast'];
      ...
      if (needVariables) {
        try {
          ...
          for (var vi = 0; vi < localVars.length; vi++) {
            ...
            lintVarsById[localVars[vi].id] = localVars[vi];
          }
        } catch (e) { /* ignore */ }
      }
      ...
      function walkNode(node, depth) {
            ...
            if (fills && fills.length > 0) {
              for (var fci = 0; fci < fills.length; fci++) {
                if (fills[fci].type === 'SOLID' && fills[fci].visible !== false) {
                  ...
                  // Resolve bound variable to get actual rendered color (not base paint black)
                  var fillColorBinding = fills[fci].boundVariables && fills[fci].boundVariables.color;
                  if (fillColorBinding && fillColorBinding.id) {
                    var resolvedFg = resolveAliasChainColor(fillColorBinding.id, lintVarsById, 0);
                    if (resolvedFg) fg = resolvedFg;
                  }
                  var bg = lintGetEffectiveBg(node);
                  // Also resolve bg variable if bound
                  var bgFillBinding = bg && bg.boundVariableColorId;
                  if (bgFillBinding) {
                    var resolvedBg = resolveAliasChainColor(bgFillBinding, lintVarsById, 0);
                    if (resolvedBg) { bg.r = resolvedBg.r; bg.g = resolvedBg.g; bg.b = resolvedBg.b; }
                  }
        ...
        // wcag-non-text-contrast: SC 1.4.11 — stroke/border vs background (3:1 minimum)
        if (activeRuleSet['wcag-non-text-contrast'] && nodeType !== 'TEXT' && !isPage && !truncated) {
          try {
            var ntcStrokes = node.strokes;
            if (ntcStrokes && ntcStrokes.length > 0) {
              for (var ntci = 0; ntci < ntcStrokes.length; ntci++) {
                var ntcStroke = ntcStrokes[ntci];
                if (ntcStroke.type === 'SOLID' && ntcStroke.visible !== false) {
                  var ntcFg = ntcStroke.color;
                  // Resolve bound variable for stroke color
                  var ntcStrokeBinding = ntcStroke.boundVariables && ntcStroke.boundVariables.color;
                  if (ntcStrokeBinding && ntcStrokeBinding.id) {
                    var ntcResolved = resolveAliasChainColor(ntcStrokeBinding.id, lintVarsById, 0);
                    if (ntcResolved) ntcFg = ntcResolved;
                  }
                  var ntcBg = lintGetEffectiveBg(node);
                  var ntcRatio = lintContrastRatio(ntcFg.r, ntcFg.g, ntcFg.b, ntcBg.r, ntcBg.g, ntcBg.b);
                  if (ntcRatio < 3.0) {
                    if (totalFindings < maxFindings) {
                      findings['wcag-non-text-contrast'].push({
                        ...
                        name: nodeName,
                        type: nodeType,
                        ratio: ntcRatio.toFixed(1) + ':1',
                        required: '3.0:1',
                        stroke: lintRgbToHex(ntcFg.r, ntcFg.g, ntcFg.b),
                        bg: lintRgbToHex(ntcBg.r, ntcBg.g, ntcBg.b)
                      });
                      totalFindings++;
                    } else {
                      truncated = true;
                  ...
                  break; // Only check first visible solid stroke
                }
              }
            }
          } catch (e) { /* slot sublayer */ }
