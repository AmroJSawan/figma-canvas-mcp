 ...
 */
function generateComponentDescriptionFixes(data) {
    ...
    for (const unit of classification.scorableUnits) {
        if (!unit.description || unit.description.trim().length === 0) {
            ...
            const context = pathParts.length > 1 ? pathParts.slice(0, -1).join("/") : "";
 ...
 */
function generateVariableDescriptionFixes(data) {
 ...
 */
function generateComponentCasingFixes(data) {
    const classification = classifyComponents(data);
    const operations = [];
    for (const unit of classification.scorableUnits) {
        ...
        if (allPascal)
            continue;
        const fixedSegments = segments.map((seg) => PASCAL_CASE_RE.test(seg) ? seg : toPascalCase(seg));
 ...
 */
function generateBooleanPrefixFixes(data) {
    const operations = [];
    for (const variable of data.variables) {
        ...
        if (BOOLEAN_PREFIX_RE.test(leaf))
            continue;
 ...
 */
function generateDelimiterConsistencyFixes(data) {
