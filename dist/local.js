 ...
 * via the WebSocket Canvas Bridge plugin.
 *
 ...
 *
 * Requirements:
 * - Canvas Bridge plugin open in Figma (Plugins → Development → Figma Canvas Bridge)
...
import { StyleValueResolver } from "./core/enrichment/style-resolver.js";
...
function setupStablePluginDir(sourcePluginDir) {
    try {
        const stableDir = join(homedir(), ".figma-canvas-mcp", "plugin");
...
If Design Systems Assistant MCP is not available, install it from: https://github.com/southleft/design-systems-mcp`,
    ...
    async getDesktopConnector() {
        ...
        throw new Error("Cannot connect to Figma Desktop.\n\n" +
            "Open the Canvas Bridge plugin in Figma (Plugins → Development → Figma Canvas Bridge).\n" +
    ...
    /**
     * Resolve the path to the Canvas Bridge plugin manifest.
    ...
    getPluginPath() {
        ...
        try {
            ...
            // From dist/local.js → go up to package root, then into plugin
            const packageRoot = dirname(dirname(thisFile));
            const manifestPath = resolve(packageRoot, "plugin", "manifest.json");
    ...
    async ensureInitialized() {
        try {
            ...
            if (this.browserManager && this.consoleMonitor) {
                ...
                else if (this.browserManager.isRunning() &&
                    !this.consoleMonitor.getStatus().isMonitoring) {
    ...
    registerTools() {
            ...
            try {
                ...
                if (this.consoleMonitor?.getStatus().isMonitoring) {
                    ...
                    try {
                        ...
                        throw new Error("No console monitoring available. Open the Canvas Bridge plugin in Figma for console capture.");
                    }
                }
                const responseData = {
                    logs,
                ...
                if (source === "websocket") {
                    responseData.ai_instruction =
                        "Console logs captured via WebSocket Bridge (plugin sandbox only). These logs include output from the Canvas Bridge plugin's code.js context.";
                }
                if (logs.length === 0) {
                    if (source === "websocket") {
                        responseData.ai_instruction =
                            "No console logs captured yet via WebSocket. The Canvas Bridge plugin is connected and monitoring. Plugin console output (console.log/warn/error from code.js) will appear here automatically. Try running a design operation that triggers plugin logging.";
                    }
                    else {
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                troubleshooting: [
                                    "Open the Canvas Bridge plugin in Figma for WebSocket-based console capture",
                                    "Ensure the Canvas Bridge plugin is open and connected in Figma",
                                ],
                            }),
                        },
                    ],
        ...
        this.server.tool("figma_take_screenshot", `Export an image of the current Figma page or specific node via REST API. Returns an image URL (valid 30 days). Use for visual validation after design changes — check alignment, spacing, proportions. Pass nodeId to target specific elements. For components, prefer figma_get_component_image.`, {
            ...
            try {
                ...
                if (!currentUrl) {
                    throw new Error("No Figma file open. Either provide a nodeId parameter, call figma_navigate, or ensure the Canvas Bridge plugin is connected.");
                }
        ...
        this.server.tool("figma_reload_plugin", "Reload the current Figma page/plugin to test code changes. Optionally clears console logs before reload. Use when user says: 'reload plugin', 'refresh page', 'restart plugin', 'test my changes'. Returns reload confirmation and current URL. Best for rapid iteration during plugin development.", {
            ...
            try {
                ...
                if (this.browserManager?.isRunning()) {
                    ...
                    await this.ensureInitialized();
                    if (!this.browserManager) {
                        throw new Error("No connection available. Open the Canvas Bridge plugin in Figma.");
                    }
                    if (clearConsoleBefore && this.consoleMonitor) {
                        clearedCount = this.consoleMonitor.clear();
                    }
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                troubleshooting: [
                                    "Open the Canvas Bridge plugin in Figma for WebSocket-based reload",
                                    "Ensure the Canvas Bridge plugin is open and connected in Figma",
                                ],
                            }),
                        },
            ...
            try {
                ...
                if (this.wsServer?.isClientConnected()) {
                    ...
                    if (this.consoleMonitor) {
                        ...
                        throw new Error("No console monitoring available. Open the Canvas Bridge plugin in Figma.");
                    }
                }
                const responseData = {
                    status: "cleared",
                            ...
                            content: [
                                {
                                    ...
                                    text: JSON.stringify({
                                        ...
                                        requestedFileKey,
                                        message: "The requested file is not connected via WebSocket. Open the Canvas Bridge plugin in the target file — it will auto-connect. Use figma_list_open_files to see all connected files.",
                                        ai_instruction: "The requested file is not in the connected files list. The user needs to open the Canvas Bridge plugin in the target Figma file. Once opened, it will auto-connect and appear in figma_list_open_files. Then use figma_navigate to switch to it.",
                                    }),
                                },
                            ],
                        };
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                troubleshooting: [
                                    "In WebSocket mode: navigate manually in Figma and ensure Canvas Bridge plugin is open",
                                    "Ensure the Canvas Bridge plugin is open in the target file",
                                ],
                            }),
                        },
                    ],
        ...
        this.server.tool("figma_get_status", "Check connection status to Figma Desktop. Reports transport status and connection health via the Canvas Bridge plugin (WebSocket transport).", {}, async () => {
            try {
                // Check WebSocket availability
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                currentFileName: currentFileName ||
                                    "(unable to retrieve - Canvas Bridge may need to be opened)",
                                ...
                                setup: {
                                    ...
                                    message: activeTransport === "websocket"
                                        ...
                                        : this.wsStartupError?.code === "EADDRINUSE"
                                            ...
                                            : this.wsActualPort !== null && this.wsActualPort !== this.wsPreferredPort
                                                ? `❌ WebSocket server running on port ${this.wsActualPort} (fallback) but no plugin connected. Restart the Canvas Bridge plugin in Figma to reconnect.`
                                    ...
                                    setupInstructions: !setupValid
                                        ? this.wsStartupError?.code === "EADDRINUSE"
                                            ...
                                            : {
                                                instructions: `Open the Canvas Bridge plugin in Figma (Plugins → Development → Figma Canvas Bridge). No special launch flags needed.${this.getPluginPath() ? ' Plugin manifest: ' + this.getPluginPath() : ''}`,
                                            }
                                        : undefined,
                                    ai_instruction: !setupValid
                                        ? this.wsStartupError?.code === "EADDRINUSE"
                                            ...
                                            : this.wsActualPort !== null && this.wsActualPort !== this.wsPreferredPort
                                                ? `Server is running on fallback port ${this.wsActualPort} (port ${this.wsPreferredPort} was taken by another instance). The Canvas Bridge plugin is not connected. TELL THE USER: Close and reopen the Canvas Bridge plugin in Figma to reconnect. The plugin's bootloader will automatically scan all ports in the range.`
                                                : `No connection to Figma Desktop. Open the Canvas Bridge plugin in Figma to connect.${this.getPluginPath() ? ' Plugin manifest: ' + this.getPluginPath() : ''}`
                                        : activeTransport === "websocket"
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                currentUrl,
                                fileName: fileName ||
                                    "(unknown - Canvas Bridge may need to be restarted)",
                                timestamp: Date.now(),
                                message: fileName
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                hint: "Open the Canvas Bridge plugin in Figma",
                            }),
                        },
                    ],
                    isError: true,
        ...
        this.server.tool("figma_get_selection", "Get the currently selected nodes in Figma. Returns node IDs, names, types, and dimensions. WebSocket-only — requires Canvas Bridge plugin. Use this to understand what the user is pointing at instead of asking them to describe it.", {
            verbose: z
                .boolean()
                .optional()
                .default(false)
                ...
                if (!this.wsServer?.isClientConnected()) {
                    return {
                        content: [{
                                type: "text",
                                text: JSON.stringify({
                                    error: "WebSocket not connected. Open the Canvas Bridge plugin in Figma.",
                                    selection: null,
                                }),
                            }],
                        isError: true,
        ...
        // Tool: Get node visual state (fills, strokes, bound variable IDs)
        this.server.tool("figma_get_node_state", "Read fills, strokes, and their bound variable IDs from any node. " +
            ...
            "NOT for variable values (use figma_get_variables). " +
            "Requires Desktop Bridge connection.", {
            nodeId: z.string().describe("Node ID (e.g. '15:203'). Get from figma_get_selection or figma_search_components.")
        }, async ({ nodeId }) => {
            try {
                const connector = await this.getDesktopConnector();
                const result = await connector.getNodeState(nodeId);
                if (!result || result.success === false) {
                    return {
                        content: [{ type: "text", text: JSON.stringify({ error: result?.error || "Unknown error", hint: "Check that the nodeId is valid. Call figma_get_selection to get current IDs." }) }],
                        isError: true,
                    };
                }
                ...
                return {
                    content: [{ type: "text", text: JSON.stringify({ error: error instanceof Error ? error.message : String(error), hint: "Node IDs are session-specific. Call figma_get_selection to get fresh IDs." }) }],
                    isError: true,
                };
            }
        }, { annotations: { readOnlyHint: true }, title: "Get Node Visual State" });
        // Tool: Get visual state for multiple nodes in one call
        this.server.tool("figma_batch_get_node_states", "Read fills, strokes, and bound variable IDs from multiple nodes in one call. " +
            "Use instead of calling figma_get_node_state in a loop — 10-50x faster for bulk audits. " +
            "Per-node errors are returned inline (nodes not found return an error field). " +
            "Requires Desktop Bridge connection.", {
            nodeIds: z.array(z.string()).describe("Array of node IDs to inspect (e.g. ['15:203', '15:204']).")
        }, async ({ nodeIds }) => {
            try {
                const connector = await this.getDesktopConnector();
                const result = await connector.getBatchNodeStates(nodeIds);
                if (!result || result.success === false) {
                    return {
                        content: [{ type: "text", text: JSON.stringify({ error: result?.error || "Unknown error", hint: "Check that nodeIds are valid. Call figma_get_selection to get current IDs." }) }],
                        isError: true,
                    };
                }
                return { content: [{ type: "text", text: JSON.stringify(result) }] };
        ...
        }, { annotations: { readOnlyHint: true }, title: "Batch Get Node Visual States" });
        // Tool: Resolve a variable's alias chain to its concrete value
        this.server.tool("figma_resolve_variable", "Resolve a variable's alias chain to its concrete value (hex color, number, string, boolean). " +
            "Traverses VARIABLE_ALIAS chains across collections to the final concrete value. " +
            "Returns resolved value per mode (Light/Dark etc.). " +
            "Use before figma_set_fills to verify what color a token resolves to. " +
            "NOT for listing all variables (use figma_get_variables). " +
            "Requires Desktop Bridge connection.", {
            variableId: z.string().describe("Variable ID (e.g. 'VariableID:8:2258'). Get from figma_get_variables or figma_get_node_state boundVariableId fields."),
            modeId: z.string().optional().describe("Specific mode ID to resolve. Omit to resolve all modes.")
        }, async ({ variableId, modeId }) => {
            try {
                const connector = await this.getDesktopConnector();
                const varsResult = await connector.getVariablesFromPluginUI();
                if (!varsResult || varsResult.success === false) {
                    return {
                        content: [{ type: "text", text: JSON.stringify({ error: "Could not fetch variables from plugin. Ensure Canvas Bridge plugin is open.", hint: "Open the Canvas Bridge plugin in Figma, then retry." }) }],
                        isError: true,
                    };
                }
                // Build lookup maps
                ...
                const variables = varsResult.variables || [];
                const collections = varsResult.variableCollections || [];
                for (const v of variables) {
                    allVariables.set(v.id, v);
                }
                for (const c of collections) {
                    allCollections.set(c.id, c);
                }
                const targetVariable = allVariables.get(variableId);
                if (!targetVariable) {
                    return {
                        content: [{ type: "text", text: JSON.stringify({ error: `Variable not found: ${variableId}`, hint: "Call figma_get_variables to list available variable IDs." }) }],
                        isError: true,
                    };
                }
                const resolver = new StyleValueResolver(logger);
                const resolvedValues = {};
                const modesToResolve = modeId
                    ? [modeId]
                    : Object.keys(targetVariable.valuesByMode || {});
                // Build the alias chain (for informational purposes)
                const chain = [];
                let cur = targetVariable;
                let chainDepth = 0;
                while (cur && chainDepth < 10) {
                    chain.push({ id: cur.id, name: cur.name });
                    const firstModeVal = Object.values(cur.valuesByMode || {})[0];
                    if (firstModeVal && typeof firstModeVal === 'object' && firstModeVal.type === 'VARIABLE_ALIAS') {
                        cur = allVariables.get(firstModeVal.id);
                        chainDepth++;
                    }
                    else {
                        break;
                ...
                for (const mid of modesToResolve) {
                    resolvedValues[mid] = await resolver.resolveVariableValueForMode(targetVariable, allVariables, allCollections, mid);
                }
                const result = {
                    variableId: targetVariable.id,
                    name: targetVariable.name,
                    resolvedType: targetVariable.resolvedType,
                    chain,
                    resolvedValues,
                };
                return { content: [{ type: "text", text: JSON.stringify(result) }] };
        ...
        }, { annotations: { readOnlyHint: true }, title: "Resolve Variable Alias Chain" });
        // Tool: Get recent design changes
        this.server.tool("figma_get_design_changes", "Get recent document changes detected in Figma. Returns buffered change events including which nodes changed, whether styles were modified, and change counts. WebSocket-only — events are captured via Canvas Bridge plugin. Use this to understand what changed since you last checked.", {
            since: z
                .number()
                .optional()
        ...
        this.server.tool("figma_list_open_files", "List all Figma files currently connected via the Canvas Bridge plugin. Shows which files have the plugin open and which one is the active target for tool calls. Use figma_navigate to switch between files. WebSocket multi-client mode — each file with the Canvas Bridge plugin maintains its own connection.", {}, async () => {
            try {
                if (!this.wsServer?.isClientConnected()) {
                            ...
                            return {
                                content: [{
                                        ...
                                        text: JSON.stringify({
                                            ...
                                            message: "WebSocket not connected. Open the Canvas Bridge plugin for multi-file support.",
                                        }),
                                    }],
                            };
                        }
                    ...
                    return {
                        content: [{
                                type: "text",
                                text: JSON.stringify({
                                    error: "No files connected. Open the Canvas Bridge plugin in Figma to connect files.",
                                    files: [],
                                }),
                            }],
                        isError: true,
        ...
        this.server.tool("figma_execute", `Execute arbitrary JavaScript in Figma's plugin context with full access to the figma API. Use for complex operations not covered by other tools. Requires Canvas Bridge plugin. CAUTION: Can modify your document.
...
Layers: If your code creates helper frames, placeholder nodes, or intermediate layers that aren't part of the final result, remove them.`, {
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            hint: "Make sure the Canvas Bridge plugin is running in Figma",
                        }),
                    },
                ],
                isError: true,
        ...
        this.server.tool("figma_update_variable", "Update a single variable's value. For multiple updates, use figma_batch_update_variables instead (10-50x faster). Use figma_get_variables first for IDs. COLOR: hex '#FF0000', FLOAT: number, STRING: text, BOOLEAN: true/false. Requires Canvas Bridge plugin.", {
            variableId: z
                .string()
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                hint: "Make sure the Canvas Bridge plugin is running and the variable ID is correct",
                            }),
                        },
                    ],
                    isError: true,
        ...
        this.server.tool("figma_create_variable", "Create a single Figma variable. For multiple variables, use figma_batch_create_variables instead (10-50x faster). Use figma_get_variables first to get collection IDs. Supports COLOR, FLOAT, STRING, BOOLEAN. Requires Canvas Bridge plugin.", {
            name: z
                .string()
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                hint: "Make sure the Canvas Bridge plugin is running and the collection ID is correct",
                            }),
                        },
                    ],
                    isError: true,
        ...
        this.server.tool("figma_create_variable_collection", "Create an empty variable collection. To create a collection WITH variables and modes in one step, use figma_setup_design_tokens instead. Requires Canvas Bridge plugin.", {
            name: z
                .string()
        ...
        this.server.tool("figma_delete_variable", "Delete a Figma variable. WARNING: This is a destructive operation that cannot be undone (except with Figma's undo). Use figma_get_variables first to get variable IDs. Requires the Canvas Bridge plugin to be running.", {
            variableId: z
                .string()
        ...
        this.server.tool("figma_delete_variable_collection", "Delete a Figma variable collection and ALL its variables. WARNING: This is a destructive operation that deletes all variables in the collection and cannot be undone (except with Figma's undo). Requires the Canvas Bridge plugin to be running.", {
            collectionId: z
                .string()
        ...
        this.server.tool("figma_rename_variable", "Rename an existing Figma variable. This updates the variable's name while preserving all its values and settings. Requires the Canvas Bridge plugin to be running.", {
            variableId: z
                .string()
        ...
        this.server.tool("figma_add_mode", "Add a new mode to an existing Figma variable collection. Modes allow variables to have different values for different contexts (e.g., Light/Dark themes, device sizes). Requires the Canvas Bridge plugin to be running.", {
            collectionId: z
                .string()
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                hint: "Make sure the Canvas Bridge plugin is running, the collection ID is correct, and you haven't exceeded Figma's mode limit",
                            }),
                        },
                    ],
                    isError: true,
        ...
        this.server.tool("figma_rename_mode", "Rename an existing mode in a Figma variable collection. Requires the Canvas Bridge plugin to be running.", {
            collectionId: z
                .string()
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                hint: "Make sure the Canvas Bridge plugin is running, the collection ID and mode ID are correct",
                            }),
                        },
                    ],
                    isError: true,
        ...
        this.server.tool("figma_batch_create_variables", "Create multiple variables in one operation. Use instead of calling figma_create_variable repeatedly — up to 50x faster for bulk operations. Get collection IDs from figma_get_variables first. Requires Canvas Bridge plugin.", {
            collectionId: z
                .string()
...
return {
  ...
  results
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    hint: "Check that the collection ID is valid and the Canvas Bridge plugin is running",
                                }),
                            },
                        ],
                        isError: true,
        ...
        this.server.tool("figma_batch_update_variables", "Update multiple variable values in one operation. Use instead of calling figma_update_variable repeatedly — up to 50x faster for bulk updates. Get variable/mode IDs from figma_get_variables first. Requires Canvas Bridge plugin.", {
            updates: z
                .array(z.object({
                variableId: z
                    .string()
...
return {
  ...
  results
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                hint: "Make sure the Canvas Bridge plugin is running and variable/mode IDs are correct",
                            }),
                        },
                    ],
                    isError: true,
        ...
        this.server.tool("figma_setup_design_tokens", "Create a complete design token structure in one operation: collection, modes, and all variables. Ideal for importing CSS custom properties or design tokens into Figma. Requires Canvas Bridge plugin.", {
            collectionName: z
                .string()
                        ...
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    hint: "Check the token definitions and ensure the Canvas Bridge plugin is running",
                                }),
                            },
                        ],
                        isError: true,
...
Without libraryFileKey/libraryFileUrl, searches the currently open file (local components via Plugin API).`, {
                        ...
                        content: [
                            {
                                type: "text",
                                text: JSON.stringify({
                                    error: "Could not load design system data. Make sure the Canvas Bridge plugin is running.",
        ...
        this.server.tool("figma_instantiate_component", `Create an instance of a component from the design system — works with BOTH local and published library components.
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                ...
                                hint: "Make sure the component key is correct and the Canvas Bridge plugin is running",
                            }),
                        },
                    ],
                    isError: true,
...
**NOTE:** Requires FIGMA_ACCESS_TOKEN to be set (uses the Figma REST API to read the library file).`, {
        ...
        // Tool: Set Node Fills
        this.server.tool("figma_set_fills", "Set the fill colors on a node. Accepts hex color strings (e.g., '#FF0000') or variable IDs for design token binding. " +
            "To bind a design token: use { type: 'SOLID', variableId: 'VariableID:8:2258' }. " +
            "Get variableIds from figma_get_variables or figma_get_node_state. " +
            "Use figma_resolve_variable to verify what color a token resolves to before binding.", {
            ...
            fills: z
                ...
                z.object({
                    type: z.literal("SOLID"),
                    variableId: z.string().describe("Variable ID to bind as fill color (e.g., 'VariableID:8:2258'). " +
                        "Resolved concrete color used as base paint. " +
                        "Get IDs from figma_get_variables or figma_get_node_state boundVariableId fields."),
                    opacity: z.number().min(0).max(1).optional().describe("Opacity 0-1 (default: 1)"),
                }),
            ]))
                .describe("Array of fill objects — each either a hex color or a variable binding"),
        }, async ({ nodeId, fills }) => {
            try {
                const connector = await this.getDesktopConnector();
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                error: error instanceof Error ? error.message : String(error),
                                hint: "Check that nodeId is valid (call figma_get_selection to refresh). If using variableId, verify the ID exists via figma_get_variables.",
                            }),
                        },
                    ],
                    isError: true,
        ...
        this.server.tool("figma_set_image_fill", "Set an image fill on one or more Figma nodes. The imageData parameter accepts EITHER a base64-encoded " +
            ...
            "as raw bytes to the Figma plugin. Requires Canvas Bridge plugin.", {
        ...
        this.server.tool("figma_arrange_component_set", `Organize a component set with Figma's native purple dashed visualization. Use after creating variants, adding states (hover/disabled/pressed), or when component sets need cleanup.
...
`;
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                error: error instanceof Error ? error.message : String(error),
                                hint: "Make sure the Canvas Bridge plugin is running and a component set exists.",
                            }),
                        },
                    ],
                    isError: true,
        ...
        this.server.tool("figma_lint_design", "Run accessibility (WCAG) and design quality checks on the current page or a specific node tree. " +
            "Checks color contrast ratios (variable-aware — resolves token bindings to actual colors), " +
            "non-text contrast (SC 1.4.11), text sizing, touch targets, hardcoded values, detached components, " +
            ...
            "Requires Canvas Bridge plugin.", {
            nodeId: z.string().optional().describe("Node ID to lint (defaults to current page)"),
            rules: z.array(z.string()).optional().describe("Rule filter: ['all'] (default), ['wcag'], ['design-system'], ['layout'], or specific rule IDs like ['wcag-contrast', 'wcag-non-text-contrast', 'detached-component']"),
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                error: error instanceof Error ? error.message : String(error),
                                hint: "Make sure the Canvas Bridge plugin is running in your Figma file.",
                            }),
                        },
                    ],
                    isError: true,
        ...
        registerFigmaAPITools(this.server, () => this.getFigmaAPI(), () => this.getCurrentFileUrl(), () => this.consoleMonitor || null, () => this.browserManager || null, () => this.ensureInitialized(), this.variablesCache, // Pass cache for efficient variable queries
        ...
        if (process.env.ENABLE_MCP_APPS === "true") {
            registerTokenBrowserApp(this.server, async (fileUrl) => {
                ...
                if (!url) {
                    throw new Error("No Figma file URL available. Either pass a fileUrl, call figma_navigate, or ensure the Canvas Bridge plugin is connected.");
                }
            ...
            registerDesignSystemDashboardApp(this.server, async (fileUrl) => {
                ...
                if (!variablesAvailable) {
                    if (desktopBridgeFailed && restApiFailed) {
                        variableError =
                            "Canvas Bridge plugin not connected and REST API requires Enterprise plan. Please open the Canvas Bridge plugin in Figma to enable variable/token analysis.";
                    }
                    else if (desktopBridgeFailed) {
                        variableError =
                            "Canvas Bridge plugin not connected. Please open the Canvas Bridge plugin in Figma to enable variable/token analysis.";
                    }
                    else if (restApiFailed) {
                        variableError =
                            "REST API requires Figma Enterprise plan. Connect the Canvas Bridge plugin in Figma for variable/token access.";
                    }
                    else if (!desktopBridgeAttempted && !restApiAttempted) {
                        variableError =
                            "No variable fetch methods available. Connect the Canvas Bridge plugin in Figma.";
                    }
                }
    ...
    async start() {
        try {
            ...
            try {
                const thisFile = fileURLToPath(import.meta.url);
                const packageRoot = dirname(dirname(thisFile));
                const sourcePluginDir = resolve(packageRoot, "plugin");
            ...
            if (this.wsServer) {
                ...
                this.wsServer.on("fileConnected", (data) => {
                    logger.info({ fileKey: data.fileKey, fileName: data.fileName }, "Canvas Bridge plugin connected via WebSocket");
                });
                this.wsServer.on("fileDisconnected", (data) => {
                    logger.info({ fileKey: data.fileKey, fileName: data.fileName }, "Canvas Bridge plugin disconnected from WebSocket");
                });
...
if (currentFile === entryFile) {
    ...
    if (process.argv.includes("--print-path")) {
        try {
            ...
            const sourceDir = resolve(packageRoot, "plugin");
            ...
            const stableDir = join(homedir(), ".figma-canvas-mcp", "plugin", "manifest.json");
