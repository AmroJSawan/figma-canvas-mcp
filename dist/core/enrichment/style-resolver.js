...
export class StyleValueResolver {
    ...
    /**
     * Resolve a variable's value for a specific mode, handling alias chains.
     * When crossing variable collections, matches by mode NAME since modeIds are collection-scoped.
     * allCollections: Map<collectionId, { modes: [{modeId, name}] }>
     */
    async resolveVariableValueForMode(variable, allVariables, allCollections, modeId, maxDepth = 10, currentDepth = 0) {
        if (currentDepth >= maxDepth) {
            return null;
        }
        const cacheKey = `var:${variable.id}:${modeId}`;
        if (this.variableCache.has(cacheKey)) {
            return this.variableCache.get(cacheKey);
        }
        try {
            // Get value for requested mode, fall back to first available mode
            ...
            if (typeof value === "object" && value.type === "VARIABLE_ALIAS") {
                ...
                // Cross-collection: look up mode by NAME in the target collection
                let targetModeId = modeId;
                if (targetVariable.variableCollectionId !== variable.variableCollectionId && allCollections) {
                    const sourceCollection = allCollections.get(variable.variableCollectionId);
                    const targetCollection = allCollections.get(targetVariable.variableCollectionId);
                    if (sourceCollection && targetCollection) {
                        const sourceModeName = sourceCollection.modes?.find(m => m.modeId === modeId)?.name;
                        if (sourceModeName) {
                            const matchedMode = targetCollection.modes?.find(m => m.name === sourceModeName);
                            targetModeId = matchedMode?.modeId ?? Object.keys(targetVariable.valuesByMode || {})[0] ?? modeId;
                        }
                    }
                }
                const resolvedValue = await this.resolveVariableValueForMode(targetVariable, allVariables, allCollections, targetModeId, maxDepth, currentDepth + 1);
