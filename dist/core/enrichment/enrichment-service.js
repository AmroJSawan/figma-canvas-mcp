...
export class EnrichmentService {
    ...
    async enrichVariables(variables, fileKey, options = {}, fileData) {
        ...
        try {
            ...
            for (const variable of variables) {
                ...
                // Resolve values for all modes (each mode resolved independently)
                if (options.include_exports !== false) {
                    const collectionsMap = this.extractCollectionsMap(data);
                    const resolved_values = {};
                    for (const [modeId] of Object.entries(variable.valuesByMode || {})) {
                        const resolvedValue = await this.styleResolver.resolveVariableValueForMode(variable, variablesMap, collectionsMap, modeId, options.max_depth);
    ...
    /**
     * Extract variable collections as a Map keyed by collection ID.
     * Each entry has shape: { id, name, modes: [{modeId, name}] }
     */
    extractCollectionsMap(fileData) {
        const collectionsMap = new Map();
        if (fileData.variableCollections) {
            for (const [id, collection] of Object.entries(fileData.variableCollections)) {
