 ...
 *
 * Creates a WebSocket server that multiple Canvas Bridge plugin instances connect to.
...
function loadPluginUIContent() {
    const candidates = [
        // ESM import.meta.url: reliable path regardless of cwd
        join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'plugin', 'ui-full.html'),
        // ESM runtime: dist/core/ → ../../plugin/
        typeof __dirname !== 'undefined'
            ? join(__dirname, '..', '..', 'plugin', 'ui-full.html')
            : join(process.cwd(), 'plugin', 'ui-full.html'),
    ...
    return '<html><body><p>Plugin UI not found. Please reinstall figma-canvas-mcp.</p></body></html>';
}
...
export class FigmaWebSocketServer extends EventEmitter {
    ...
    sendCommand(method, params = {}, timeoutMs = 15000, targetFileKey) {
        return new Promise((resolve, reject) => {
            ...
            if (!fileKey) {
                reject(new Error('No active file connected. Make sure the Canvas Bridge plugin is open in Figma.'));
                return;
            }
            const client = this.clients.get(fileKey);
            if (!client || client.ws.readyState !== WebSocket.OPEN) {
                reject(new Error('No WebSocket client connected. Make sure the Canvas Bridge plugin is open in Figma.'));
                return;
            }
